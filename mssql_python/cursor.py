...
# NVARCHAR/VARCHAR inline limit; this triggers NVARCHAR(MAX)/VARCHAR(MAX) + DAE
...
class Cursor:
    ...
    def __init__(self, connection, timeout: int = 0) -> None:
        ...
        # internal: index of the next row the driver will return (0-based)
        self._next_row_index = 0
        self._has_result_set = False  # Track if we have an active result set
        # Track if we need to skip incrementing the row index
    ...
    def _initialize_description(self, column_metadata=None):
        ...
        for i, col in enumerate(column_metadata):
            ...
            description.append((
                ...
                # precision - should match ColumnSize
                col["ColumnSize"],
    ...
    def rownumber(self):
        ...
        # Will be None until first fetch, then 0, 1, 2, etc.
    ...
    def _increment_rownumber(self):
        ...
        if self._has_result_set:
            ...
            raise InterfaceError(
                "Cannot increment rownumber: no active result set.", "No active result set.")
    ...
    def _decrement_rownumber(self):
        ...
        if self._has_result_set and self._rownumber >= 0:
            ...
            raise InterfaceError(
                "Cannot decrement rownumber: no active result set.", "No active result set.")
        ...
        *parameters,
        ...
        if self._timeout > 0:
            try:
                ...
                check_error(ddbc_sql_const.SQL_HANDLE_STMT.value,
                            self.hstmt, ret)
    ...
    def _prepare_metadata_result_set(self, column_metadata=None, fallback_description=None, specialized_mapping=None):
        ...
        if column_metadata is None:
            ...
            try:
                ...
                log('error',
                    f"Driver interface error during metadata retrieval: {e}")
            except Exception as e:
    ...
    def primaryKeys(self, table, catalog=None, schema=None):
        ...
        retcode = ddbc_bindings.DDBCSQLPrimaryKeys(
            self.hstmt, catalog, schema, table)
        check_error(ddbc_sql_const.SQL_HANDLE_STMT.value, self.hstmt, retcode)
        ...
        for row in seq_of_parameters:
            ...
            if first_row:
                ...
                if len(row) != num_params:
                    raise ValueError(
                        "Inconsistent parameter row size in executemany()")
    ...
    def executemany(self, operation: str, seq_of_parameters: list) -> None:
        ...
        sample_row = seq_of_parameters[0] if hasattr(
            seq_of_parameters, '__getitem__') else next(iter(seq_of_parameters))
        ...
        for col_index in range(param_count):
            column = [row[col_index] for row in seq_of_parameters] if hasattr(
                seq_of_parameters, '__getitem__') else []
            ...
            if self._inputsizes and col_index < len(self._inputsizes):
                ...
                column = [row[col_index] for row in seq_of_parameters] if hasattr(
                    seq_of_parameters, '__getitem__') else []
                sample_value, min_val, max_val = self._compute_column_type(
                    column)
        ...
        log('debug', "Executing batch query with %d parameter sets:\n%s",
            len(seq_of_parameters), "\n".join(f"  {i+1}: {tuple(p) if isinstance(p, (list, tuple)) else p}" for i, p in enumerate(
    ...
    def _execute_tables(self, stmt_handle, catalog_name=None, schema_name=None, table_name=None,
        ...
        if stmt_handle:
            self.messages.extend(
                ddbc_bindings.DDBCSQLGetAllDiagRecords(stmt_handle))
