...
PARAM_TEST_DATA = [
    TEST_DATA,
    (2, 0, 0, 0, 0, 0, 0.0, "test1", time(0, 0, 0),
     datetime(2024, 1, 1, 0, 0, 0), date(2024, 1, 1), 0.0),
    (3, 1, 1, 1, 1, 1, 1.1, "test2", time(1, 1, 1),
     datetime(2024, 2, 2, 1, 1, 1), date(2024, 2, 2), 1.1),
    (4, 0, 127, 32767, 9223372036854775807, 2147483647, 1.23456789, "test3", time(
        12, 34, 56), datetime(2024, 5, 20, 12, 34, 56, 123000), date(2024, 5, 20), 1.23456789)
]
...
def test_empty_string_handling(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "SELECT text_col FROM #pytest_empty_string WHERE id = 1")
        row = cursor.fetchone()
...
def test_empty_binary_handling(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute("INSERT INTO #pytest_empty_binary VALUES (1, 0x)")
        db_connection.commit()

        # Fetch the empty binary - this would previously cause assertion failure
        cursor.execute(
            "SELECT binary_col FROM #pytest_empty_binary WHERE id = 1")
        row = cursor.fetchone()
        assert row is not None, "Should return a row"
...
def test_mixed_empty_and_null_values(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute("INSERT INTO #pytest_empty_vs_null VALUES (1, '', 0x)")
        # NULL values
        cursor.execute(
...
def test_empty_string_edge_cases(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "CREATE TABLE #pytest_empty_edge (id INT, data NVARCHAR(MAX))")
        db_connection.commit()
...
def test_insert_id_column(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "CREATE TABLE #pytest_single_column (id INTEGER PRIMARY KEY)")
        db_connection.commit()
        cursor.execute(
            "INSERT INTO #pytest_single_column (id) VALUES (?)", [1])
        db_connection.commit()
...
def test_insert_bit_column(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute(
            "INSERT INTO #pytest_single_column (bit_column) VALUES (?)", [1])
        db_connection.commit()
...
def test_insert_time_column(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute("INSERT INTO #pytest_single_column (time_column) VALUES (?)", [
                       time(12, 34, 56)])
        db_connection.commit()
...
def test_insert_datetime_column(cursor, db_connection):
    ...
    try:
        drop_table_if_exists(cursor, "#pytest_single_column")
        cursor.execute(
            "CREATE TABLE #pytest_single_column (datetime_column DATETIME)")
        db_connection.commit()
        cursor.execute("INSERT INTO #pytest_single_column (datetime_column) VALUES (?)", [
                       datetime(2024, 5, 20, 12, 34, 56, 123000)])
        db_connection.commit()
        cursor.execute("SELECT datetime_column FROM #pytest_single_column")
        row = cursor.fetchone()
        assert row[0] == datetime(
            2024, 5, 20, 12, 34, 56, 123000), "Datetime column insertion/fetch failed"
    except Exception as e:
...
def test_insert_datetime2_column(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute("INSERT INTO #pytest_single_column (datetime2_column) VALUES (?)", [
                       datetime(2024, 5, 20, 12, 34, 56, 123456)])
        db_connection.commit()
        cursor.execute("SELECT datetime2_column FROM #pytest_single_column")
        row = cursor.fetchone()
        assert row[0] == datetime(
            2024, 5, 20, 12, 34, 56, 123456), "Datetime2 column insertion/fetch failed"
    except Exception as e:
...
def test_insert_smalldatetime_column(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute("INSERT INTO #pytest_single_column (smalldatetime_column) VALUES (?)", [
                       datetime(2024, 5, 20, 12, 34)])
        db_connection.commit()
        cursor.execute(
            "SELECT smalldatetime_column FROM #pytest_single_column")
        row = cursor.fetchone()
        assert row[0] == datetime(
            2024, 5, 20, 12, 34), "Smalldatetime column insertion/fetch failed"
    except Exception as e:
...
def test_insert_date_column(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute("INSERT INTO #pytest_single_column (date_column) VALUES (?)", [
                       date(2024, 5, 20)])
        db_connection.commit()
        cursor.execute("SELECT date_column FROM #pytest_single_column")
        row = cursor.fetchone()
        assert row[0] == date(
            2024, 5, 20), "Date column insertion/fetch failed"
    except Exception as e:
...
def test_insert_real_column(cursor, db_connection):
    ...
    try:
        ...
        row = cursor.fetchone()
        assert abs(
            row[0] - 1.23456789) < 1e-8, "Real column insertion/fetch failed"
    except Exception as e:
...
def test_insert_decimal_column(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute("INSERT INTO #pytest_single_column (decimal_column) VALUES (?)", [
        ...
        assert row[0] == decimal.Decimal(123.45).quantize(
            decimal.Decimal('0.00')), "Decimal column insertion/fetch failed"
        ...
        row = cursor.fetchone()
        assert row[0] == decimal.Decimal(-123.45).quantize(
            decimal.Decimal('0.00')), "Negative Decimal insertion/fetch failed"
    except Exception as e:
...
def test_insert_tinyint_column(cursor, db_connection):
    ...
    try:
        cursor.execute(
            "CREATE TABLE #pytest_single_column (tinyint_column TINYINT)")
        db_connection.commit()
        cursor.execute(
...
def test_insert_smallint_column(cursor, db_connection):
    ...
    try:
        cursor.execute(
            "CREATE TABLE #pytest_single_column (smallint_column SMALLINT)")
        db_connection.commit()
        cursor.execute(
...
def test_insert_bigint_column(cursor, db_connection):
    ...
    try:
        cursor.execute(
            "CREATE TABLE #pytest_single_column (bigint_column BIGINT)")
        db_connection.commit()
        cursor.execute("INSERT INTO #pytest_single_column (bigint_column) VALUES (?)", [
                       9223372036854775807])
        db_connection.commit()
...
def test_insert_integer_column(cursor, db_connection):
    ...
    try:
        cursor.execute(
            "CREATE TABLE #pytest_single_column (integer_column INTEGER)")
        db_connection.commit()
        cursor.execute(
            "INSERT INTO #pytest_single_column (integer_column) VALUES (?)", [2147483647])
        db_connection.commit()
...
def test_insert_float_column(cursor, db_connection):
    ...
    try:
        cursor.execute(
            "CREATE TABLE #pytest_single_column (float_column FLOAT)")
        db_connection.commit()
        cursor.execute(
...
def test_varchar_full_capacity(cursor, db_connection):
    ...
    try:
        cursor.execute(
            "CREATE TABLE #pytest_varchar_test (varchar_column VARCHAR(9))")
        db_connection.commit()
        cursor.execute(
            "INSERT INTO #pytest_varchar_test (varchar_column) VALUES (?)", ['123456789'])
        db_connection.commit()
        # fetchone test
        ...
        rows = cursor.fetchall()
        assert rows[0] == [
            '123456789'], "SQL_VARCHAR parsing failed for fetchall"
    except Exception as e:
...
def test_wvarchar_full_capacity(cursor, db_connection):
    ...
    try:
        ...
        rows = cursor.fetchall()
        assert rows[0] == [
            '123456'], "SQL_WVARCHAR parsing failed for fetchall"
    except Exception as e:
...
def test_varbinary_full_capacity(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute("INSERT INTO #pytest_varbinary_test (varbinary_column) VALUES (?)", bytearray(
            "12345", 'utf-8'))
        cursor.execute("INSERT INTO #pytest_varbinary_test (varbinary_column) VALUES (?)", bytes(
            "12345678", 'utf-8'))  # Full capacity
        db_connection.commit()
        expectedRows = 2
        # fetchone test
        ...
        ) == None, "varbinary_column is expected to have only {} rows".format(expectedRows)
        assert rows[0] == [
            bytes("12345", 'utf-8')], "SQL_VARBINARY parsing failed for fetchone - row 0"
        assert rows[1] == [
            bytes("12345678", 'utf-8')], "SQL_VARBINARY parsing failed for fetchone - row 1"
        # fetchall test
        cursor.execute("SELECT varbinary_column FROM #pytest_varbinary_test")
        rows = cursor.fetchall()
        assert rows[0] == [
            bytes("12345", 'utf-8')], "SQL_VARBINARY parsing failed for fetchall - row 0"
        assert rows[1] == [
            bytes("12345678", 'utf-8')], "SQL_VARBINARY parsing failed for fetchall - row 1"
    except Exception as e:
...
def test_varbinary_max(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute("INSERT INTO #pytest_varbinary_test (varbinary_column) VALUES (?), (?)", [
        ...
        assert rows[0] == [bytearray(
            "ABCDEF", 'utf-8')], "SQL_VARBINARY parsing failed for fetchone - row 0"
        assert rows[1] == [
            bytes("123!@#", 'utf-8')], "SQL_VARBINARY parsing failed for fetchone - row 1"
        # fetchall test
        cursor.execute("SELECT varbinary_column FROM #pytest_varbinary_test")
        rows = cursor.fetchall()
        assert rows[0] == [bytearray(
            "ABCDEF", 'utf-8')], "SQL_VARBINARY parsing failed for fetchall - row 0"
        assert rows[1] == [
            bytes("123!@#", 'utf-8')], "SQL_VARBINARY parsing failed for fetchall - row 1"
    except Exception as e:
        pytest.fail(f"SQL_VARBINARY parsing test failed: {e}")
    finally:
        cursor.execute("DROP TABLE #pytest_varbinary_test")
...
def test_longvarchar(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute("INSERT INTO #pytest_longvarchar_test (longvarchar_column) VALUES (?), (?)", [
                       "ABCDEFGHI", None])
        db_connection.commit()
        expectedRows = 2
        # fetchone test
        cursor.execute(
            "SELECT longvarchar_column FROM #pytest_longvarchar_test")
        rows = []
        for i in range(0, expectedRows):
            rows.append(cursor.fetchone())
        assert cursor.fetchone(
        ) == None, "longvarchar_column is expected to have only {} rows".format(expectedRows)
        assert rows[0] == [
            "ABCDEFGHI"], "SQL_LONGVARCHAR parsing failed for fetchone - row 0"
        assert rows[1] == [
            None], "SQL_LONGVARCHAR parsing failed for fetchone - row 1"
        # fetchall test
        cursor.execute(
            "SELECT longvarchar_column FROM #pytest_longvarchar_test")
        rows = cursor.fetchall()
        assert rows[0] == [
            "ABCDEFGHI"], "SQL_LONGVARCHAR parsing failed for fetchall - row 0"
        assert rows[1] == [
            None], "SQL_LONGVARCHAR parsing failed for fetchall - row 1"
    except Exception as e:
...
def test_longwvarchar(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute("INSERT INTO #pytest_longwvarchar_test (longwvarchar_column) VALUES (?), (?)", [
                       "ABCDEFGHI", None])
        db_connection.commit()
        expectedRows = 2
        # fetchone test
        cursor.execute(
            "SELECT longwvarchar_column FROM #pytest_longwvarchar_test")
        rows = []
        for i in range(0, expectedRows):
            rows.append(cursor.fetchone())
        assert cursor.fetchone(
        ) == None, "longwvarchar_column is expected to have only {} rows".format(expectedRows)
        assert rows[0] == [
            "ABCDEFGHI"], "SQL_LONGWVARCHAR parsing failed for fetchone - row 0"
        assert rows[1] == [
            None], "SQL_LONGWVARCHAR parsing failed for fetchone - row 1"
        # fetchall test
        cursor.execute(
            "SELECT longwvarchar_column FROM #pytest_longwvarchar_test")
        rows = cursor.fetchall()
        assert rows[0] == [
            "ABCDEFGHI"], "SQL_LONGWVARCHAR parsing failed for fetchall - row 0"
        assert rows[1] == [
            None], "SQL_LONGWVARCHAR parsing failed for fetchall - row 1"
    except Exception as e:
...
def test_longvarbinary(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute("INSERT INTO #pytest_longvarbinary_test (longvarbinary_column) VALUES (?), (?)", [
                       bytearray("ABCDEFGHI", 'utf-8'), bytes("123!@#", 'utf-8')])
        db_connection.commit()
        expectedRows = 2  # Only 2 rows are inserted
        # fetchone test
        cursor.execute(
            "SELECT longvarbinary_column FROM #pytest_longvarbinary_test")
        rows = []
        for i in range(0, expectedRows):
            rows.append(cursor.fetchone())
        assert cursor.fetchone(
        ) == None, "longvarbinary_column is expected to have only {} rows".format(expectedRows)
        assert rows[0] == [bytearray(
            "ABCDEFGHI", 'utf-8')], "SQL_LONGVARBINARY parsing failed for fetchone - row 0"
        assert rows[1] == [bytes(
            "123!@#", 'utf-8')], "SQL_LONGVARBINARY parsing failed for fetchone - row 1"
        # fetchall test
        cursor.execute(
            "SELECT longvarbinary_column FROM #pytest_longvarbinary_test")
        rows = cursor.fetchall()
        assert rows[0] == [bytearray(
            "ABCDEFGHI", 'utf-8')], "SQL_LONGVARBINARY parsing failed for fetchall - row 0"
        assert rows[1] == [bytes(
            "123!@#", 'utf-8')], "SQL_LONGVARBINARY parsing failed for fetchall - row 1"
    except Exception as e:
...
def test_rowcount(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #pytest_test_rowcount (name) VALUES ('JohnDoe1');")
        assert cursor.rowcount == 1, "Rowcount should be 1 after first insert"

        cursor.execute(
            "INSERT INTO #pytest_test_rowcount (name) VALUES ('JohnDoe2');")
        assert cursor.rowcount == 1, "Rowcount should be 1 after second insert"

        cursor.execute(
            "INSERT INTO #pytest_test_rowcount (name) VALUES ('JohnDoe3');")
...
def test_rowcount_executemany(cursor, db_connection):
    ...
    try:
        ...
        cursor.executemany(
            "INSERT INTO #pytest_test_rowcount (name) VALUES (?)", data)
...
def test_fetchmany_with_arraysize(cursor, db_connection):
    ...
    assert len(
        rows) == 3, "fetchmany with arraysize returned incorrect number of rows"
...
def test_fetchall(cursor):
    ...
    assert len(rows) == len(
        PARAM_TEST_DATA), "Incorrect number of rows returned"
...
def test_execute_many(cursor, db_connection):
    ...
    cursor.executemany(
        "INSERT INTO #pytest_all_data_types (id) VALUES (?)", data)
...
def test_executemany_empty_strings(cursor, db_connection):
    ...
    try:
        ...
        cursor.executemany(
            "INSERT INTO #pytest_empty_batch VALUES (?, ?)", test_data)
        db_connection.commit()
        ...
        for i, (actual, expected_row) in enumerate(zip(results, expected)):
            ...
            assert actual[1] == expected_row[
                1], f"Row {i}: Data mismatch - expected '{expected_row[1]}', got '{actual[1]}'"
    except Exception as e:
...
def test_executemany_empty_strings_various_types(cursor, db_connection):
    ...
    try:
        ...
        pytest.fail(
            f"Executemany with empty strings in various types failed: {e}")
    finally:
...
def test_executemany_unicode_and_empty_strings(cursor, db_connection):
    ...
    try:
        ...
        # Execute the batch insert
        cursor.executemany(
            "INSERT INTO #pytest_unicode_test VALUES (?, ?)", test_data)
        db_connection.commit()
...
def test_executemany_large_batch_with_empty_strings(cursor, db_connection):
    ...
    try:
        ...
        # Execute the batch insert
        cursor.executemany(
            "INSERT INTO #pytest_large_batch VALUES (?, ?)", test_data)
        db_connection.commit()
        ...
        for actual, expected in zip(results, expected_subset):
            ...
            assert actual[1] == expected[
                1], f"Data mismatch for ID {actual[0]}: expected '{expected[1]}', got '{actual[1]}'"
    except Exception as e:
        pytest.fail(
            f"Executemany with large batch and empty strings failed: {e}")
    finally:
...
def test_executemany_compare_with_execute(cursor, db_connection):
    ...
    try:
        ...
        for row_data in test_data:
            cursor.execute(
                "INSERT INTO #pytest_compare_test VALUES (?, ?)", row_data)
        db_connection.commit()
        ...
        cursor.execute("DELETE FROM #pytest_compare_test")
        cursor.executemany(
            "INSERT INTO #pytest_compare_test VALUES (?, ?)", test_data)
        db_connection.commit()
        ...
        assert len(execute_results) == len(
            executemany_results), "Row count mismatch between execute and executemany"
...
def test_executemany_edge_cases_empty_strings(cursor, db_connection):
    ...
    try:
        ...
        assert len(results) == len(
            test_data), f"Expected {len(test_data)} rows, got {len(results)}"

        # Check each row
        for i, (actual, expected_row) in enumerate(zip(results, test_data)):
            assert actual[0] == expected_row[0], f"Row {i}: ID mismatch"
            assert actual[1] == expected_row[
                1], f"Row {i}: VARCHAR mismatch - expected '{repr(expected_row[1])}', got '{repr(actual[1])}'"
            assert actual[2] == expected_row[
                2], f"Row {i}: NVARCHAR mismatch - expected '{repr(expected_row[2])}', got '{repr(actual[2])}'"
    except Exception as e:
...
def test_executemany_null_vs_empty_string(cursor, db_connection):
    ...
    try:
        ...
        # Execute the batch insert
        cursor.executemany(
            "INSERT INTO #pytest_null_vs_empty VALUES (?, ?)", test_data)
        db_connection.commit()

        # Verify the data was inserted correctly
        cursor.execute(
            "SELECT id, data FROM #pytest_null_vs_empty ORDER BY id")
        results = cursor.fetchall()
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM #pytest_null_vs_empty WHERE data IS NULL")
        null_count = cursor.fetchone()[0]
        assert null_count == 3, f"Expected 3 NULL values, got {null_count}"

        cursor.execute(
            "SELECT COUNT(*) FROM #pytest_null_vs_empty WHERE data = ''")
...
def test_executemany_binary_data_edge_cases(cursor, db_connection):
    ...
    try:
        ...
        # Execute the batch insert
        cursor.executemany(
            "INSERT INTO #pytest_binary_test VALUES (?, ?)", test_data)
        db_connection.commit()

        # Verify the data was inserted correctly
        cursor.execute(
            "SELECT id, binary_data FROM #pytest_binary_test ORDER BY id")
        results = cursor.fetchall()
        ...
        for i, (actual, expected_row) in enumerate(zip(results, test_data)):
            ...
            if expected_row[1] is None:
                ...
                assert actual[1] == expected_row[
                    1], f"Row {i}: Binary data mismatch  expected {expected_row[1]}, got {actual[1]}"
    except Exception as e:
...
def test_executemany_strings_of_various_lengths(cursor, db_connection):
    ...
    try:
        cursor.execute(
            "CREATE TABLE #pytest_varied_strings (val NVARCHAR(50))")
        data = [("a",), ("abcd",), ("abc",)]
        cursor.executemany(
            "INSERT INTO #pytest_varied_strings VALUES (?)", data)
        db_connection.commit()
...
def test_join_operations(cursor):
    ...
    try:
        ...
        assert rows[0] == [
            'Alice', 'HR', 'Project A'], "Join operation returned incorrect data for row 1"
        assert rows[1] == ['Bob', 'Engineering',
                           'Project B'], "Join operation returned incorrect data for row 2"
        assert rows[2] == ['Charlie', 'HR',
                           'Project C'], "Join operation returned incorrect data for row 3"
    except Exception as e:
...
def test_join_operations_with_parameters(cursor):
    ...
    try:
        ...
        rows = cursor.fetchall()
        assert len(
            rows) == 2, "Join operation with parameters returned incorrect number of rows"
        assert rows[0] == [
            'Alice', 'HR', 'Project A'], "Join operation with parameters returned incorrect data for row 1"
        assert rows[1] == ['Bob', 'Engineering',
                           'Project B'], "Join operation with parameters returned incorrect data for row 2"
    except Exception as e:
...
def test_execute_stored_procedure_with_parameters(cursor):
    ...
    try:
        ...
        rows = cursor.fetchall()
        assert len(
            rows) == 1, "Stored procedure with parameters returned incorrect number of rows"
        assert rows[0] == [
            'Alice', 'Project A'], "Stored procedure with parameters returned incorrect data"
    except Exception as e:
...
def test_execute_stored_procedure_without_parameters(cursor):
    ...
    try:
        ...
        rows = cursor.fetchall()
        assert len(
            rows) == 1, "Stored procedure without parameters returned incorrect number of rows"
        assert rows[0] == [
            'Bob', 'Project B'], "Stored procedure without parameters returned incorrect data"
    except Exception as e:
        pytest.fail(
            f"Stored procedure execution without parameters failed: {e}")
...
def test_cursor_description(cursor):
    ...
    assert len(desc) == len(
        expected_description), "Description length mismatch"
...
def test_parse_datetime(cursor, db_connection):
    ...
    try:
        cursor.execute(
            "CREATE TABLE #pytest_datetime_test (datetime_column DATETIME)")
        db_connection.commit()
        cursor.execute("INSERT INTO #pytest_datetime_test (datetime_column) VALUES (?)", [
                       '2024-05-20T12:34:56.123'])
        db_connection.commit()
        cursor.execute("SELECT datetime_column FROM #pytest_datetime_test")
        row = cursor.fetchone()
        assert row[0] == datetime(
            2024, 5, 20, 12, 34, 56, 123000), "Datetime parsing failed"
    except Exception as e:
...
def test_parse_smalldatetime(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute("INSERT INTO #pytest_smalldatetime_test (smalldatetime_column) VALUES (?)", [
                       '2024-05-20 12:34'])
        db_connection.commit()
        cursor.execute(
            "SELECT smalldatetime_column FROM #pytest_smalldatetime_test")
        row = cursor.fetchone()
        assert row[0] == datetime(
            2024, 5, 20, 12, 34), "Smalldatetime parsing failed"
    except Exception as e:
...
def test_parse_datetime2(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute("INSERT INTO #pytest_datetime2_test (datetime2_column) VALUES (?)", [
                       '2024-05-20 12:34:56.123456'])
        db_connection.commit()
        cursor.execute("SELECT datetime2_column FROM #pytest_datetime2_test")
        row = cursor.fetchone()
        assert row[0] == datetime(
            2024, 5, 20, 12, 34, 56, 123456), "Datetime2 parsing failed"
    except Exception as e:
...
def test_none(cursor, db_connection):
    ...
    try:
        cursor.execute(
            "CREATE TABLE #pytest_none_test (none_column NVARCHAR(255))")
        db_connection.commit()
        cursor.execute(
            "INSERT INTO #pytest_none_test (none_column) VALUES (?)", [None])
        db_connection.commit()
...
def test_boolean(cursor, db_connection):
    ...
    try:
        cursor.execute(
            "CREATE TABLE #pytest_boolean_test (boolean_column BIT)")
        db_connection.commit()
        cursor.execute(
...
def test_sql_wvarchar(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute("INSERT INTO #pytest_wvarchar_test (wvarchar_column) VALUES (?)", [
                       'nvarchar data'])
        db_connection.commit()
        cursor.execute("SELECT wvarchar_column FROM #pytest_wvarchar_test")
        row = cursor.fetchone()
...
def test_sql_varchar(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute("INSERT INTO #pytest_varchar_test (varchar_column) VALUES (?)", [
                       'varchar data'])
        db_connection.commit()
        cursor.execute("SELECT varchar_column FROM #pytest_varchar_test")
        row = cursor.fetchone()
...
def test_row_comparison_with_list(cursor, db_connection):
    ...
    try:
        ...
        row = cursor.fetchone()
        assert row == [10, 'test_string',
                       3.14], "Row did not compare equal to matching list"
        assert row != [10, 'different',
                       3.14], "Row compared equal to non-matching list"
        ...
        cursor.execute(
            "SELECT * FROM #pytest_row_comparison_test WHERE col1 = 10")
        row1 = cursor.fetchone()
        cursor.execute(
            "SELECT * FROM #pytest_row_comparison_test WHERE col1 = 20")
        row2 = cursor.fetchone()
        assert row1 != row2, "Different rows should not be equal"

        # Test fetchmany row comparison with lists
        cursor.execute(
            "SELECT * FROM #pytest_row_comparison_test ORDER BY col1")
        rows = cursor.fetchmany(2)
        assert len(rows) == 2, "Should have fetched 2 rows"
        assert rows[0] == [10, 'test_string',
                           3.14], "First row didn't match expected list"
        assert rows[1] == [20, 'other_string',
                           2.71], "Second row didn't match expected list"
...
def test_row_column_mapping(cursor, db_connection):
    ...
    try:
        ...
        assert getattr(
            row, "Complex Name!") == 42, "Complex column name access failed"
...
def test_lowercase_setting_after_cursor_creation(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "CREATE TABLE #test_lowercase_after (UserName VARCHAR(50))")
        db_connection.commit()
...
def test_cursor_context_manager_autocommit_true(db_connection):
    ...
    try:
        ...
        cursor.execute(
            "CREATE TABLE #test_autocommit (id INT, value NVARCHAR(50))")
        cursor.close()
...
def test_cursor_context_manager_no_auto_commit(db_connection):
    ...
    try:
        ...
        cursor = db_connection.cursor()
        cursor.execute(
            "CREATE TABLE #test_no_autocommit (id INT, value NVARCHAR(50))")
        db_connection.commit()
        cursor.close()
...
def test_cursor_context_manager_exception_handling(db_connection):
    ...
    try:
        ...
        cursor = db_connection.cursor()
        cursor.execute(
            "CREATE TABLE #test_exception (id INT, value NVARCHAR(50))")
        cursor.execute(
...
def test_cursor_context_manager_transaction_behavior(db_connection):
    ...
    try:
        ...
        cursor = db_connection.cursor()
        cursor.execute(
            "CREATE TABLE #test_tx_behavior (id INT, value NVARCHAR(50))")
        db_connection.commit()
        cursor.close()
        ...
        with db_connection.cursor() as cursor:
            ...
            assert isinstance(count_after_context,
                              int), "First query should work"
...
def test_cursor_context_manager_multiple_operations(db_connection):
    ...
    try:
        ...
        with db_connection.cursor() as cursor:
            # Create table
            cursor.execute(
                "CREATE TABLE #test_multiple (id INT, value NVARCHAR(50))")

            # Multiple inserts
            cursor.execute(
                "INSERT INTO #test_multiple (id, value) VALUES (1, 'first')")
            cursor.execute(
                "INSERT INTO #test_multiple (id, value) VALUES (2, 'second')")
            cursor.execute(
                "INSERT INTO #test_multiple (id, value) VALUES (3, 'third')")
...
def test_execute_returns_self(cursor):
    ...
    assert id(result) == id(
        cursor), "Returned cursor should be the same object"
...
def test_execute_fetchone_chaining(cursor, db_connection):
    ...
    try:
        # Create test table
        cursor.execute(
            "CREATE TABLE #test_chaining (id INT, value NVARCHAR(50))")
        db_connection.commit()
        ...
        row = cursor.execute(
            "SELECT id, value FROM #test_chaining WHERE id = ?", 1).fetchone()
        assert row is not None, "Should return a row"
        ...
        row = cursor.execute(
            "SELECT id, value FROM #test_chaining WHERE id = ?", 999).fetchone()
...
def test_execute_fetchall_chaining(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #test_chaining (id, value) VALUES (1, 'first')")
        cursor.execute(
            "INSERT INTO #test_chaining (id, value) VALUES (2, 'second')")
        cursor.execute(
            "INSERT INTO #test_chaining (id, value) VALUES (3, 'third')")
        db_connection.commit()

        # Test execute().fetchall() chaining
        rows = cursor.execute(
            "SELECT id, value FROM #test_chaining ORDER BY id").fetchall()
        ...
        rows = cursor.execute(
            "SELECT id, value FROM #test_chaining WHERE id > ?", 1).fetchall()
...
def test_execute_fetchmany_chaining(cursor, db_connection):
    ...
    try:
        ...
        rows = cursor.execute(
            "SELECT id, value FROM #test_chaining ORDER BY id").fetchmany(3)
        ...
        cursor.arraysize = 2
        rows = cursor.execute(
            "SELECT id, value FROM #test_chaining ORDER BY id").fetchmany()
...
def test_execute_rowcount_chaining(cursor, db_connection):
    ...
    try:
        ...
        count = cursor.execute(
            "INSERT INTO #test_chaining (id, value) VALUES (?, ?)", 1, "test").rowcount
        ...
        count = cursor.execute(
            "UPDATE #test_chaining SET value = ? WHERE id > ?", "updated", 2).rowcount
        assert count == 2, "UPDATE should affect 2 rows"

        # Test DELETE rowcount chaining
        count = cursor.execute(
            "DELETE FROM #test_chaining WHERE id = ?", 1).rowcount
...
def test_execute_description_chaining(cursor):
    ...
    description = cursor.execute(
        "SELECT 1 as int_col, 'test' as str_col, GETDATE() as date_col").description
    ...
    description = cursor.execute(
        "SELECT database_id, name FROM sys.databases WHERE database_id = 1").description
...
def test_chaining_with_iteration(cursor, db_connection):
    ...
    try:
        # Create test table
        cursor.execute(
            "CREATE TABLE #test_iteration (id INT, name NVARCHAR(50))")
        db_connection.commit()
        ...
        for i, name in enumerate(names, 1):
            cursor.execute(
                "INSERT INTO #test_iteration VALUES (?, ?)", i, name)
        db_connection.commit()
...
def test_cursor_next_functionality(cursor, db_connection):
    ...
    try:
        ...
        for id_val, name in test_data:
            cursor.execute(
                "INSERT INTO #test_next VALUES (?, ?)", id_val, name)
        db_connection.commit()
...
def test_cursor_next_with_different_data_types(cursor, db_connection):
    ...
    try:
        ...
        assert row[5] == datetime(
            2024, 1, 15, 10, 30, 0), "Datetime should match"
...
def test_execute_chaining_compatibility_examples(cursor, db_connection):
    ...
    try:
        ...
        deleted_count = cursor.execute(
            "DELETE FROM #users WHERE status = ?", "inactive").rowcount
...
def test_rownumber_basic_functionality(cursor, db_connection):
    ...
    try:
        # Create test table
        cursor.execute(
            "CREATE TABLE #test_rownumber (id INT, value VARCHAR(50))")
        db_connection.commit()

        # Insert test data
        for i in range(5):
            cursor.execute(
                "INSERT INTO #test_rownumber VALUES (?, ?)", i, f"value_{i}")
        db_connection.commit()
        ...
        assert initial_rownumber == - \
            1, f"Initial rownumber should be -1, got {initial_rownumber}"
...
def test_cursor_rownumber_mixed_fetches(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "SELECT * FROM #pytest_rownumber_mixed_test ORDER BY id")
...
def test_cursor_rownumber_empty_results(cursor, db_connection):
    ...
    try:
        ...
        try:
            cursor.execute(
                "DROP TABLE IF EXISTS #pytest_rownumber_empty_results")
            db_connection.commit()
        except:
            pass
...
def test_rownumber_warning_logged(cursor, db_connection):
    ...
    try:
        ...
        if logger:
            ...
            rownumber = cursor.rownumber
            assert rownumber == - \
                1, f"Expected rownumber -1 before fetch, got {rownumber}"
...
def test_cursor_rownumber_fetchall(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "SELECT * FROM #pytest_rownumber_all_test WHERE id > 100")
...
def test_nextset_with_different_result_sizes_safe(cursor, db_connection):
    ...
    try:
        ...
        cursor.executemany(
            "INSERT INTO #test_nextset_sizes VALUES (?, ?)", test_data)
        db_connection.commit()
        ...
        try:
            ...
            try:
                ...
                import warnings
                warnings.warn(
                    f"nextset() test skipped due to driver limitation: {e}")
...
def test_nextset_basic_functionality_only(cursor, db_connection):
    ...
    try:
        ...
        try:
            ...
            import warnings
            warnings.warn(
                f"Multi-statement query not supported by driver: {e}")
...
def test_nextset_memory_safety_check(cursor, db_connection):
    ...
    try:
        ...
        for i in range(3):
            cursor.execute(
                "INSERT INTO #test_nextset_memory VALUES (?)", i + 1)
        db_connection.commit()

        # Test multiple simple queries to check for memory leaks
        for iteration in range(3):
            cursor.execute(
                "SELECT value FROM #test_nextset_memory ORDER BY value")
            ...
            assert cursor.rownumber == - \
                1, f"Iteration {iteration}: rownumber should be -1 after nextset"
...
def test_nextset_error_conditions_safe(cursor, db_connection):
    ...
    try:
        ...
        try:
            ...
            assert cursor.rownumber == - \
                1, "rownumber should remain -1 even if nextset() raises exception"
...
def test_fetchval_basic_functionality(cursor, db_connection):
    ...
    try:
        ...
        assert isinstance(
            count, int), "fetchval should return integer for COUNT(*)"
    ...
    try:
        ...
        test_cases = [
            ...
            ("SELECT decimal_col FROM #pytest_fetchval_types",
             decimal.Decimal('89.12'), decimal.Decimal),
            ...
            ("SELECT datetime_col FROM #pytest_fetchval_types",
             datetime(2024, 5, 20, 12, 34, 56), datetime),
        ...
        for query, expected_value, expected_type in test_cases:
            ...
            assert isinstance(
                result, expected_type), f"fetchval should return {expected_type.__name__} for {query}"
            if isinstance(expected_value, float):
                assert abs(
                    result - expected_value) < 0.01, f"Float values should be approximately equal for {query}"
            else:
...
def test_fetchval_no_results(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "SELECT col FROM #pytest_fetchval_empty WHERE col = 999")
        result = cursor.fetchval()
...
def test_fetchval_rownumber_tracking(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "SELECT col FROM #pytest_fetchval_rownumber ORDER BY col")
        ...
        assert cursor.rownumber == initial_rownumber + \
            1, "fetchval should increment rownumber"
    ...
    try:
        ...
        for query, expected in test_cases:
            ...
            if isinstance(expected, float):
                assert abs(
                    result - expected) < 0.01, f"Aggregate function result should match for {query}"
            else:
...
def test_fetchval_performance_common_patterns(cursor, db_connection):
    ...
    try:
        ...
        # Test TOP 1 pattern
        cursor.execute(
            "SELECT TOP 1 id FROM #pytest_fetchval_perf ORDER BY id")
...
def test_cursor_rollback_basic(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #pytest_cursor_rollback VALUES (1, 'permanent')")
        cursor.commit()

        # Insert more data but don't commit
        cursor.execute(
            "INSERT INTO #pytest_cursor_rollback VALUES (2, 'temp1')")
        cursor.execute(
            "INSERT INTO #pytest_cursor_rollback VALUES (3, 'temp2')")
...
def test_cursor_commit_affects_all_cursors(db_connection):
    ...
    try:
        ...
        cursor1.execute(
            "INSERT INTO #pytest_multi_cursor VALUES (1, 'cursor1')")

        # Insert data using cursor2
        cursor2.execute(
            "INSERT INTO #pytest_multi_cursor VALUES (2, 'cursor2')")
...
def test_cursor_rollback_affects_all_cursors(db_connection):
    ...
    try:
        ...
        cursor1.execute(
            "INSERT INTO #pytest_multi_rollback VALUES (0, 'baseline')")
        cursor1.commit()  # Commit initial state

        # Insert data using both cursors
        cursor1.execute(
            "INSERT INTO #pytest_multi_rollback VALUES (1, 'cursor1')")
        cursor2.execute(
            "INSERT INTO #pytest_multi_rollback VALUES (2, 'cursor2')")
...
def test_cursor_commit_equivalent_to_connection_commit(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #pytest_commit_equiv VALUES (1, 'cursor_commit')")
        cursor.commit()

        # Verify the chained operation worked
        result = cursor.execute(
            "SELECT method FROM #pytest_commit_equiv WHERE id = 1").fetchval()
        assert result == 'cursor_commit', "Method chaining with commit should work"

        # Test 2: Use connection.commit()
        cursor.execute(
            "INSERT INTO #pytest_commit_equiv VALUES (2, 'conn_commit')")
        db_connection.commit()
...
def test_cursor_commit_with_method_chaining(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "CREATE TABLE #pytest_chaining (id INTEGER, value VARCHAR(20))")
        cursor.commit()
        ...
        # Verify the chained operation worked
        result = cursor.execute(
            "SELECT value FROM #pytest_chaining WHERE id = 1").fetchval()
        assert result == 'chained', "Method chaining with commit should work"

        # Verify rollback worked
        count = cursor.execute(
            "SELECT COUNT(*) FROM #pytest_chaining").fetchval()
...
def test_cursor_commit_error_scenarios(cursor, db_connection):
    ...
    try:
        ...
        try:
            cursor.execute(
                "INSERT INTO #pytest_commit_errors VALUES (1, 'duplicate')")
        ...
        cursor.execute(
            "INSERT INTO #pytest_commit_errors VALUES (2, 'after_error')")
        cursor.commit()
...
def test_cursor_rollback_error_scenarios(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #pytest_rollback_errors VALUES (1, 'committed')")
        cursor.commit()

        # Start a transaction with multiple operations
        cursor.execute(
            "INSERT INTO #pytest_rollback_errors VALUES (2, 'temp1')")
        cursor.execute(
            "INSERT INTO #pytest_rollback_errors VALUES (3, 'temp2')")
        cursor.execute(
        ...
        cursor.execute(
            "SELECT value FROM #pytest_rollback_errors WHERE id = 1")
...
def test_cursor_rollback_with_method_chaining(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #pytest_rollback_chaining VALUES (1, 'permanent')")
        cursor.commit()

        # Test method chaining with execute and rollback
        cursor.execute(
            "INSERT INTO #pytest_rollback_chaining VALUES (2, 'temporary')")

        # Verify temporary data is visible before rollback
        result = cursor.execute(
            "SELECT COUNT(*) FROM #pytest_rollback_chaining").fetchval()
        ...
        value = cursor.execute(
            "SELECT value FROM #pytest_rollback_chaining WHERE id = 1").fetchval()
...
def test_cursor_rollback_savepoints_simulation(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #pytest_rollback_savepoints VALUES (1, 'stage1')")
        cursor.commit()

        # Stage 2: Insert more data but don't commit
        cursor.execute(
            "INSERT INTO #pytest_rollback_savepoints VALUES (2, 'stage2')")
        cursor.execute(
            "INSERT INTO #pytest_rollback_savepoints VALUES (3, 'stage2')")
        ...
        cursor.execute(
            "INSERT INTO #pytest_rollback_savepoints VALUES (4, 'stage3')")
        cursor.execute(
            "UPDATE #pytest_rollback_savepoints SET stage = 'modified' WHERE id = 1")
        cursor.execute(
            "INSERT INTO #pytest_rollback_savepoints VALUES (5, 'stage3')")
        ...
        cursor.execute(
            "SELECT stage FROM #pytest_rollback_savepoints WHERE id = 1")
            ...
            try:
                ...
                for i in range(batch_size):
                    ...
                    if batch_num == 1 and i >= 3:  # Simulate failure in batch 1
                        ...
                        raise Exception(
                            f"Simulated error in batch {batch_num}")
                    else:
                        cursor.execute("INSERT INTO #pytest_rollback_perf VALUES (?, ?, ?)",
...
def test_cursor_rollback_equivalent_to_connection_rollback(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #pytest_rollback_equiv VALUES (2, 'conn_rollback')")
        cursor.execute("SELECT COUNT(*) FROM #pytest_rollback_equiv")
        count = cursor.fetchval()
        assert count == 1, "Data should be visible before rollback"
        ...
        # Test 3: Mix both methods
        cursor.execute(
            "INSERT INTO #pytest_rollback_equiv VALUES (3, 'mixed1')")
        cursor.rollback()  # Use cursor

        cursor.execute(
            "INSERT INTO #pytest_rollback_equiv VALUES (4, 'mixed2')")
        ...
        cursor.execute(
            "INSERT INTO #pytest_rollback_equiv VALUES (5, 'final_test')")
...
def test_cursor_rollback_nested_transactions_simulation(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "SELECT operation FROM #pytest_rollback_nested WHERE id = 1")
...
def test_cursor_rollback_data_consistency(cursor, db_connection):
    ...
    try:
        ...
        # Insert initial data
        cursor.execute(
            "INSERT INTO #pytest_rollback_customers VALUES (1, 'John Doe')")
        cursor.execute(
        ...
        cursor.execute(
            "INSERT INTO #pytest_rollback_orders VALUES (1, 1, 100.00)")
        cursor.execute(
            "INSERT INTO #pytest_rollback_orders VALUES (2, 2, 200.00)")
        cursor.execute(
            "INSERT INTO #pytest_rollback_orders VALUES (3, 3, 300.00)")
        cursor.execute(
        ...
        cursor.execute(
            "SELECT name FROM #pytest_rollback_customers WHERE id = 1")
        ...
        cursor.execute(
            "SELECT name FROM #pytest_rollback_customers ORDER BY id")
...
def test_cursor_rollback_large_transaction(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #pytest_rollback_large VALUES (0, 'baseline')")
        cursor.commit()
        ...
        total_count = cursor.fetchval()
        assert total_count == large_transaction_size + \
            1, f"Should have {large_transaction_size + 1} records before rollback"
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM #pytest_rollback_large WHERE id > 0")
...
def test_scroll_relative_basic(cursor, db_connection):
    ...
    try:
        ...
        cursor.executemany("INSERT INTO #t_scroll_rel VALUES (?)", [
                           (i,) for i in range(1, 11)])
        db_connection.commit()

        cursor.execute(
            "SELECT id FROM #t_scroll_rel ORDER BY id", scrollable=True)
...
def test_scroll_absolute_basic(cursor, db_connection):
    ...
    try:
        ...
        cursor.executemany("INSERT INTO #t_scroll_abs VALUES (?)", [
                           (i,) for i in range(1, 8)])
        db_connection.commit()

        cursor.execute(
            "SELECT id FROM #t_scroll_abs ORDER BY id", scrollable=True)
...
def test_scroll_backward_not_supported(cursor, db_connection):
    ...
    try:
        ...
        cursor.executemany("INSERT INTO #t_scroll_back VALUES (?)", [
                           (1,), (2,), (3,)])
        db_connection.commit()

        cursor.execute(
            "SELECT id FROM #t_scroll_back ORDER BY id", scrollable=True)
...
def test_scroll_on_empty_result_set_raises(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute("SELECT id FROM #t_scroll_empty", scrollable=True)
        ...
        assert cursor.fetchone(
        ) is None, "No row should be returned after absolute positioning into empty set"
...
def test_scroll_mixed_fetches_consume_correctly(db_connection):
    ...
    try:
        ...
        try:
            ...
            setup_cursor.executemany("INSERT INTO #t_scroll_mix VALUES (?)", [
                                     (i,) for i in range(1, 11)])
            db_connection.commit()
        finally:
            setup_cursor.close()
        ...
        try:
            part1_cursor.execute(
                "SELECT id FROM #t_scroll_mix ORDER BY id", scrollable=True)
...
def test_scroll_edge_cases_and_validation(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute("SELECT id FROM #t_scroll_validation", scrollable=True)
...
def test_cursor_skip_basic_functionality(cursor, db_connection):
    ...
    try:
        ...
        cursor.executemany("INSERT INTO #test_skip VALUES (?)", [
                           (i,) for i in range(1, 11)])
        db_connection.commit()
...
def test_cursor_skip_zero_is_noop(cursor, db_connection):
    ...
    try:
        ...
        cursor.executemany("INSERT INTO #test_skip_zero VALUES (?)", [
                           (i,) for i in range(1, 6)])
        db_connection.commit()

        # Execute query
        cursor.execute(
            "SELECT id FROM #test_skip_zero ORDER BY id", scrollable=True)
...
def test_cursor_skip_empty_result_set(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute("SELECT id FROM #test_skip_empty", scrollable=True)
...
def test_cursor_skip_past_end(cursor, db_connection):
    ...
    try:
        ...
        cursor.executemany("INSERT INTO #test_skip_end VALUES (?)", [
                           (i,) for i in range(1, 4)])
        db_connection.commit()

        # Execute query
        cursor.execute(
            "SELECT id FROM #test_skip_end ORDER BY id", scrollable=True)
...
def test_cursor_skip_invalid_arguments(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute("SELECT id FROM #test_skip_args", scrollable=True)
...
def test_cursor_skip_integration_with_fetch_methods(cursor, db_connection):
    ...
    try:
        ...
        cursor.executemany("INSERT INTO #test_skip_fetch VALUES (?)", [
                           (i,) for i in range(1, 11)])
        db_connection.commit()

        # Test with fetchone
        cursor.execute(
            "SELECT id FROM #test_skip_fetch ORDER BY id", scrollable=True)
        ...
        assert [r[0] for r in rows] == [
            5, 6], "After fetchmany(2) and skip(3), should get ids matching implementation"
        ...
        assert [r[0] for r in rows] == [6, 7, 8, 9,
                                        10], "After skip(5), fetchall() should get id=6-10"
...
def test_cursor_messages_preservation_across_fetches(cursor, db_connection):
    ...
    try:
        ...
        # Insert data
        cursor.execute(
            "INSERT INTO #test_messages_preservation VALUES (1), (2), (3)")
        db_connection.commit()
        ...
        # Execute query to set up result set
        cursor.execute(
            "SELECT id FROM #test_messages_preservation ORDER BY id")
        ...
        assert len(
            cursor.messages) == 0, "Messages should be cleared by execute()"
...
def test_cursor_messages_with_warnings(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #test_messages_warnings VALUES (1, 123.456)")
...
def test_cursor_messages_manual_clearing(cursor):
    ...
    del cursor.messages[:]
    assert len(
        cursor.messages) == 0, "Messages should be cleared after del cursor.messages[:]"
...
def test_cursor_messages_executemany(cursor, db_connection):
    ...
    try:
        ...
        data = [(1,), (2,), (3,)]
        cursor.executemany(
            "INSERT INTO #test_messages_executemany VALUES (?)", data)
...
def test_tables_setup(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "DROP TABLE IF EXISTS pytest_tables_schema.regular_table")
        cursor.execute(
            "DROP TABLE IF EXISTS pytest_tables_schema.another_table")
...
def test_tables_all(cursor, db_connection):
    ...
    try:
        ...
        assert hasattr(
            first_row, 'table_cat'), "Result should have table_cat column"
        assert hasattr(
            first_row, 'table_schem'), "Result should have table_schem column"
        assert hasattr(
            first_row, 'table_name'), "Result should have table_name column"
        assert hasattr(
            first_row, 'table_type'), "Result should have table_type column"
        assert hasattr(
            first_row, 'remarks'), "Result should have remarks column"
...
def test_tables_specific_table(cursor, db_connection):
    ...
    try:
        ...
        assert table.table_schem.lower(
        ) == 'pytest_tables_schema', "Schema should be 'pytest_tables_schema'"
...
def test_tables_catalog_filter(cursor, db_connection):
    ...
    try:
        ...
        assert len(
            fake_tables) == 0, "Should return empty list for non-existent catalog"
...
def test_tables_nonexistent(cursor):
    ...
    assert isinstance(
        tables_list, list), "Should return a list for non-existent table"
    ...
    assert len(
        tables_list) == 0, "Should return empty list for non-existent schema"
...
def test_tables_combined_filters(cursor, db_connection):
    ...
    try:
        ...
        assert tables_list[0].table_name.lower(
        ) == 'regular_table', "Should find regular_table"
...
def test_tables_result_processing(cursor, db_connection):
    ...
    try:
        ...
        just_tables = [
            table for table in tables_list if table.table_type == 'TABLE']
        ...
        assert len(schema_table_map['pytest_tables_schema']
                   ) == 3, "Should have 3 objects in test schema"
...
def test_tables_method_chaining(cursor, db_connection):
    ...
    try:
        ...
        assert chained_result[0].table_name.lower(
        ) == 'regular_table', "Should find regular_table"
...
def test_emoji_round_trip(cursor, db_connection):
    ...
    for text in test_inputs:
        try:
            cursor.execute(
                "INSERT INTO #pytest_emoji_test (content) OUTPUT INSERTED.id VALUES (?)", [text])
...
def test_varcharmax_transaction_rollback(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #pytest_varcharmax VALUES (?)", [rollback_str])
...
def test_nvarcharmax_transaction_rollback(cursor, db_connection):
    ...
    try:
        ...
        rollback_str = "ROLLBACK" * 2000
        cursor.execute(
            "INSERT INTO #pytest_nvarcharmax VALUES (?)", [rollback_str])
        db_connection.rollback()
        cursor.execute(
...
def test_empty_varbinary_batch_fetch(cursor, db_connection):
    ...
    try:
        ...
        assert isinstance(
            rows[0][1], bytes), "Should return bytes type for empty binary"
...
def test_empty_values_fetchmany(cursor, db_connection):
    ...
    try:
        ...
        for i, row in enumerate(rows):
            ...
            assert isinstance(
                row[2], bytes), f"Row {i+1} VARBINARY should be bytes type"
...
def test_sql_no_total_large_data_scenario(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute("INSERT INTO #pytest_large_data_no_total VALUES (1, ?, ?)", [
                       large_string, large_binary])
        cursor.execute("INSERT INTO #pytest_large_data_no_total VALUES (2, ?, ?)", [
                       large_string, large_binary])
        db_connection.commit()
        ...
        if row[0] is not None:
            assert isinstance(
                row[0], str), "Text data should be str if not None"
            assert len(row[0]) > 0, "Text data should be non-empty if not None"
        if row[1] is not None:
            assert isinstance(
                row[1], bytes), "Binary data should be bytes if not None"
            assert len(
                row[1]) > 0, "Binary data should be non-empty if not None"
        ...
        for i, row in enumerate(rows):
            if row[0] is not None:
                assert isinstance(
                    row[0], str), f"Row {i+1} text should be str if not None"
            if row[1] is not None:
                assert isinstance(
                    row[1], bytes), f"Row {i+1} binary should be bytes if not None"

        # Test fetchmany - should handle SQL_NO_TOTAL consistently
        cursor.execute(
            "SELECT large_text FROM #pytest_large_data_no_total ORDER BY id")
        many_rows = cursor.fetchmany(2)
        assert len(many_rows) == 2, "fetchmany should return 2 rows"
        ...
        for i, row in enumerate(many_rows):
            if row[0] is not None:
                assert isinstance(
                    row[0], str), f"fetchmany row {i+1} should be str if not None"

    except Exception as e:
        # Should not crash with assertion errors about dataLen
        assert "Data length must be" not in str(
            e), "Should not fail with dataLen assertion"
        assert "assert" not in str(e).lower(
        ), "Should not fail with assertion errors"
...
def test_batch_fetch_empty_values_no_assertion_failure(cursor, db_connection):
    ...
    try:
        ...
        for i, row in enumerate(all_batch_rows):
            ...
            assert isinstance(
                row[1], bytes), f"Batch row {i+1} should return bytes type"
...
def test_executemany_utf16_length_validation(cursor, db_connection):
    ...
    try:
        ...
        print(
            f"Emoji overflow test error on {platform.system()}: {error_msg[:100]}...")
...
def test_binary_data_over_8000_bytes(cursor, db_connection):
    ...
    try:
        ...
        # Verify the data was inserted correctly
        cursor.execute(
            "SELECT id, large_binary FROM #pytest_small_binary ORDER BY id")
        results = cursor.fetchall()

        assert len(results) == 2, f"Expected 2 rows, got {len(results)}"
        assert len(
            results[0][1]) == 3000, f"Expected 3000 bytes, got {len(results[0][1])}"
        assert len(
            results[1][1]) == 1000, f"Expected 1000 bytes, got {len(results[1][1])}"
...
def test_varbinarymax_insert_fetch(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "SELECT id, binary_data FROM #pytest_varbinarymax ORDER BY id")
        rows = []
        while True:
            row = cursor.fetchone()
            if row is None:
        ...
        assert len(rows) == len(
            test_data), f"Expected {len(test_data)} rows, got {len(rows)}"
        ...
        for i, (expected_id, expected_data) in enumerate(test_data):
            ...
            assert isinstance(
                fetched_data, bytes), f"Row {i+1} expected bytes, got {type(fetched_data)}"
...
def test_all_empty_binaries(cursor, db_connection):
    ...
    try:
        ...
        for i, row in enumerate(results, 1):
            ...
            assert isinstance(
                row[1], bytes), f"Row {i} should return bytes type, got {type(row[1])}"
...
def test_mixed_bytes_and_bytearray_types(cursor, db_connection):
    ...
    try:
        ...
        for i, (row, expected) in enumerate(zip(results, expected_values)):
            ...
            assert isinstance(
                row[1], bytes), f"Row {i+1} should return bytes type, got {type(row[1])}"
...
def test_binary_mostly_small_one_large(cursor, db_connection):
    ...
    try:
        ...
        for i, (row, expected_len) in enumerate(zip(results, expected_lengths)):
            assert row[0] == i + 1, f"ID mismatch for row {i+1}"
            assert len(
                row[1]) == expected_len, f"Row {i+1}: expected length {expected_len}, got {len(row[1])}"
        ...
        cursor.execute(
            "SELECT binary_data FROM #pytest_mixed_size_binary WHERE id = 4")
        large_result = cursor.fetchone()
        assert len(large_result[0]
                   ) == 3500, "Large binary should be 3,500 bytes"
...
def test_varcharmax_boundary(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute(
            "INSERT INTO #pytest_varcharmax VALUES (?)", [boundary_str])
        db_connection.commit()
...
def test_varcharmax_large(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute(
            "INSERT INTO #pytest_varcharmax VALUES (?)", [large_str])
        db_connection.commit()
...
def test_nvarcharmax_boundary(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute(
            "INSERT INTO #pytest_nvarcharmax VALUES (?)", [boundary_str])
        db_connection.commit()
...
def test_nvarcharmax_large(cursor, db_connection):
    ...
    try:
        ...
        db_connection.commit()
        cursor.execute(
            "INSERT INTO #pytest_nvarcharmax VALUES (?)", [large_str])
        db_connection.commit()
...
def test_money_smallmoney_insert_fetch(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "SELECT m, sm, d, n FROM #pytest_money_test ORDER BY id")
        results = cursor.fetchall()
        ...
        for i, (row, exp) in enumerate(zip(results, expected)):
            for j, (val, exp_val) in enumerate(zip(row, exp), 1):
                if exp_val is None:
                    ...
                    assert isinstance(
                        val, decimal.Decimal), f"Row {i+1} col{j}: expected Decimal, got {type(val)}"
...
def test_money_smallmoney_roundtrip(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #pytest_money_test (m, sm) VALUES (?, ?)", values)
        db_connection.commit()
        ...
        for i, (val, exp_val) in enumerate(zip(row, values), 1):
            ...
            assert isinstance(
                val, decimal.Decimal), f"col{i} should be Decimal, got {type(val)}"
    ...
    try:
        ...
        for i, (row, exp_row) in enumerate(zip(results, expected), 1):
            for j, (val, exp_val) in enumerate(zip(row, exp_row), 1):
                ...
                assert isinstance(
                    val, decimal.Decimal), f"Row {i} col{j} should be Decimal, got {type(val)}"
...
def test_money_smallmoney_invalid_values(cursor, db_connection):
    ...
    try:
        ...
        # Out of range MONEY
        with pytest.raises(Exception):
            cursor.execute("INSERT INTO #pytest_money_test (m) VALUES (?)",
                           (decimal.Decimal("922337203685477.5808"),))

        # Out of range SMALLMONEY
        with pytest.raises(Exception):
            cursor.execute("INSERT INTO #pytest_money_test (sm) VALUES (?)",
                           (decimal.Decimal("214748.3648"),))
...
def test_money_smallmoney_executemany_null_handling(cursor, db_connection):
    ...
    try:
        ...
        cursor.executemany(
            "INSERT INTO #pytest_money_test (m, sm) VALUES (?, ?)", rows)
        db_connection.commit()
...
def test_money_smallmoney_out_of_range_low(cursor, db_connection):
    ...
    try:
        drop_table_if_exists(cursor, "#pytest_money_test")
        cursor.execute(
            "CREATE TABLE #pytest_money_test (m MONEY, sm SMALLMONEY)")
        db_connection.commit()
...
def test_insert_multiple_uuids(cursor, db_connection):
    ...
    try:
        ...
        for retrieved_uuid, retrieved_desc in rows:
            assert isinstance(
                retrieved_uuid, uuid.UUID), f"Expected uuid.UUID, got {type(retrieved_uuid)}"
...
def test_fetchmany_uuids(cursor, db_connection):
    ...
    try:
        ...
        assert len(fetched_rows) == len(
            uuids_to_insert), "Fetched row count mismatch"
...
def test_invalid_uuid_inserts(cursor, db_connection):
    ...
    try:
        ...
        for val in invalid_values:
            with pytest.raises(Exception):
                cursor.execute(
                    f"INSERT INTO {table_name} (id) VALUES (?)", [val])
                db_connection.commit()
    finally:
        cursor.execute(f"DROP TABLE IF EXISTS {table_name}")
        db_connection.commit()
...
def test_duplicate_uuid_inserts(cursor, db_connection):
    ...
    try:
        cursor.execute(f"DROP TABLE IF EXISTS {table_name}")
        cursor.execute(
            f"CREATE TABLE {table_name} (id UNIQUEIDENTIFIER PRIMARY KEY)")
        db_connection.commit()
...
def test_executemany_uuid_insert_and_select(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            f"SELECT id, description FROM {table_name} ORDER BY description")
        rows = cursor.fetchall()

        # Verify the number of fetched rows
        assert len(rows) == len(
            data_to_insert), "Number of fetched rows does not match."
...
def test_executemany_uuid_roundtrip_fixed_value(cursor, db_connection):
    ...
    try:
        ...
        # Fetch back
        cursor.execute(
            f"SELECT id, description FROM {table_name} WHERE description = ?", description)
        row = cursor.fetchone()
...
def test_decimal_separator_calculations(cursor, db_connection):
    ...
    try:
        ...
        assert row.sum_result == decimal.Decimal(
            '16.00'), "Sum calculation incorrect with default separator"
        ...
        row = cursor.fetchone()
        assert row.sum_result == decimal.Decimal(
            '16.00'), "Sum calculation affected by separator change"

        # But string representation should use comma
        assert '16,00' in str(
            row), "Sum result not formatted with comma in string representation"
...
def test_decimal_separator_function(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "SELECT id, decimal_value FROM #pytest_decimal_separator_test")
        row = cursor.fetchone()
        default_str = str(row)
...
def test_decimal_separator_basic_functionality():
    ...
    try:
        # Test default value
        assert mssql_python.getDecimalSeparator(
        ...
        with pytest.raises(ValueError):
            mssql_python.setDecimalSeparator(
                'too_long')  # More than one character
...
def test_lowercase_attribute(cursor, db_connection):
    ...
    try:
        ...
        try:
            ...
            if drop_cursor:
                drop_cursor.execute(
                    "DROP TABLE IF EXISTS #pytest_lowercase_test")
                db_connection.commit()
                drop_cursor.close()
        except Exception as e:
...
def test_datetimeoffset_read_write(cursor, db_connection):
    ...
    try:
        test_cases = [
            ...
            datetime(2023, 10, 26, 10, 30, 0, tzinfo=timezone(
        ...
        cursor.execute(
            "DROP TABLE IF EXISTS #pytest_datetimeoffset_read_write;")
        db_connection.commit()
...
def test_datetimeoffset_max_min_offsets(cursor, db_connection):
    ...
    try:
        ...
        test_cases = [
            (1, datetime(2025, 1, 1, 12, 0, 0, tzinfo=timezone(
                timedelta(hours=14)))),  # max offset
            (2, datetime(2025, 1, 1, 12, 0, 0, tzinfo=timezone(
                timedelta(hours=-14)))),  # min offset
        ]
...
def test_datetimeoffset_invalid_offsets(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "DROP TABLE IF EXISTS #pytest_datetimeoffset_invalid_offsets;")
        db_connection.commit()
...
def test_datetimeoffset_dst_transitions(cursor, db_connection):
    ...
    try:
        ...
        dst_test_cases = [
            (1, datetime(2025, 3, 9, 1, 59, 59, tzinfo=timezone(
                timedelta(hours=-5)))),  # Just before spring forward
            # Just after spring forward
            (2, datetime(2025, 3, 9, 3, 0, 0, tzinfo=timezone(timedelta(hours=-4)))),
            (3, datetime(2025, 11, 2, 1, 59, 59, tzinfo=timezone(
                timedelta(hours=-4)))),  # Just before fall back
            (4, datetime(2025, 11, 2, 1, 0, 0, tzinfo=timezone(
                timedelta(hours=-5)))),   # Just after fall back
        ]
        ...
        cursor.execute(
            "DROP TABLE IF EXISTS #pytest_datetimeoffset_dst_transitions;")
        db_connection.commit()
...
def test_datetimeoffset_leap_second(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #pytest_datetimeoffset_leap_second (id, dto_column) VALUES (?, ?);", 1, leap_second_sim)
        db_connection.commit()

        row = cursor.execute(
            "SELECT dto_column FROM #pytest_datetimeoffset_leap_second;").fetchone()
        assert row[0].tzinfo is not None
    finally:
        cursor.execute(
            "DROP TABLE IF EXISTS #pytest_datetimeoffset_leap_second;")
        db_connection.commit()
...
def test_datetimeoffset_malformed_input(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "DROP TABLE IF EXISTS #pytest_datetimeoffset_malformed_input;")
        db_connection.commit()
...
def test_datetimeoffset_executemany(cursor, db_connection):
    ...
    try:
        ...
        param_list = [(i, python_dt) for i, (_, python_dt)
...
def test_datetimeoffset_native_vs_string_simple(cursor, db_connection):
    ...
    try:
        ...
        test_rows = [
            (1, datetime(2025, 5, 14, 12, 35, 52, 501000,
             tzinfo=timezone(timedelta(hours=1)))),
            (2, datetime(2025, 5, 14, 15, 20, 30, 123000,
...
def test_cursor_setinputsizes_override_inference(db_connection):
    ...
    cursor.setinputsizes([
        ...
        # Force short string to NVARCHAR(MAX)
        (mssql_python.SQL_WVARCHAR, 8000, 0)
    ])
...
def test_setinputsizes_sql_injection_protection(db_connection):
    ...
    # Create a test table
    cursor.execute(
        "CREATE TABLE #test_sql_injection (id INT, name VARCHAR(100))")
...
def test_gettypeinfo_all_types(cursor):
    ...
    assert any(
        'VARCHAR' in name for name in type_names), "VARCHAR type should be in results"
    assert any(
        'INT' in name for name in type_names), "INTEGER type should be in results"

    # Verify first row has expected columns
    first_row = type_info[0]
    assert hasattr(
        first_row, 'type_name'), "Result should have type_name column"
    assert hasattr(
        first_row, 'data_type'), "Result should have data_type column"
    assert hasattr(
        first_row, 'column_size'), "Result should have column_size column"
...
def test_gettypeinfo_specific_type(cursor):
    ...
    varchar_info = cursor.getTypeInfo(
        ConstantsDDBC.SQL_VARCHAR.value).fetchall()

    # Verify we got results specific to VARCHAR
    assert varchar_info is not None, "getTypeInfo(SQL_VARCHAR) should return results"
    assert len(
        varchar_info) > 0, "getTypeInfo(SQL_VARCHAR) should return at least one row"
...
def test_gettypeinfo_result_structure(cursor):
    ...
    for column in required_columns:
        assert hasattr(
            first_row, column), f"Result missing required column: {column}"
...
def test_gettypeinfo_numeric_type(cursor):
    ...
    decimal_info = cursor.getTypeInfo(
        ConstantsDDBC.SQL_DECIMAL.value).fetchall()
...
def test_gettypeinfo_datetime_types(cursor):
    ...
    datetime_info = cursor.getTypeInfo(
        ConstantsDDBC.SQL_TYPE_TIMESTAMP.value).fetchall()

    # Verify we got datetime-related results
    assert len(
        datetime_info) > 0, "getTypeInfo for TIMESTAMP should return results"
...
def test_gettypeinfo_multiple_calls(cursor):
    ...
    varchar_info = cursor.getTypeInfo(
        ConstantsDDBC.SQL_VARCHAR.value).fetchall()
    assert len(
        varchar_info) > 0, "Second call to getTypeInfo should return results"
    ...
    assert len(all_types) > len(
        varchar_info), "All types should return more rows than specific type"
...
def test_gettypeinfo_cached_results(cursor):
    ...
    first_result = cursor.getTypeInfo(
        ConstantsDDBC.SQL_VARCHAR.value).fetchall()
    ...
    start_time = time.time()
    second_result = cursor.getTypeInfo(
        ConstantsDDBC.SQL_VARCHAR.value).fetchall()
    second_duration = time.time() - start_time

    # Results should be consistent
    assert len(first_result) == len(
        second_result), "Multiple calls should return same number of results"
...
def test_procedures_all(cursor, db_connection):
    ...
    try:
        ...
        first_row = procs[0]
        assert hasattr(
            first_row, 'procedure_cat'), "Result should have procedure_cat column"
        assert hasattr(
            first_row, 'procedure_schem'), "Result should have procedure_schem column"
        assert hasattr(
            first_row, 'procedure_name'), "Result should have procedure_name column"
        assert hasattr(
            first_row, 'num_input_params'), "Result should have num_input_params column"
        assert hasattr(
            first_row, 'num_output_params'), "Result should have num_output_params column"
        assert hasattr(
            first_row, 'num_result_sets'), "Result should have num_result_sets column"
        assert hasattr(
            first_row, 'remarks'), "Result should have remarks column"
        assert hasattr(
            first_row, 'procedure_type'), "Result should have procedure_type column"
...
def test_procedures_specific(cursor, db_connection):
    ...
    try:
        ...
        procs = cursor.procedures(
            procedure='test_proc1', schema='pytest_proc_schema').fetchall()
...
def test_procedures_nonexistent(cursor):
    ...
    procs = cursor.procedures(
        procedure='nonexistent_procedure_xyz123').fetchall()

    # Should return empty list, not error
    assert isinstance(
        procs, list), "Should return a list for non-existent procedure"
...
def test_procedures_catalog_filter(cursor, db_connection):
    ...
    try:
        ...
        procs = cursor.procedures(
            catalog=current_db, schema='pytest_proc_schema').fetchall()
        ...
        fake_procs = cursor.procedures(
            catalog='nonexistent_db_xyz123').fetchall()
        assert len(
            fake_procs) == 0, "Should return empty list for non-existent catalog"
...
def test_procedures_with_parameters(cursor, db_connection):
    ...
    try:
        ...
        # Get procedure info
        procs = cursor.procedures(
            procedure='test_params_proc', schema='pytest_proc_schema').fetchall()
        ...
        assert hasattr(
            proc, 'num_input_params'), "Result should have num_input_params column"
        assert hasattr(
            proc, 'num_output_params'), "Result should have num_output_params column"
        ...
        cursor.execute(
            "DROP PROCEDURE IF EXISTS pytest_proc_schema.test_params_proc")
        db_connection.commit()
...
def test_procedures_result_set_info(cursor, db_connection):
    ...
    try:
        ...
        procs = cursor.procedures(
            schema='pytest_proc_schema', procedure='test_%').fetchall()
        ...
        for proc in procs:
            assert hasattr(
                proc, 'num_result_sets'), "Result should have num_result_sets column"
        ...
        cursor.execute(
            "DROP PROCEDURE IF EXISTS pytest_proc_schema.test_no_results")
        cursor.execute(
            "DROP PROCEDURE IF EXISTS pytest_proc_schema.test_one_result")
        cursor.execute(
...
def test_procedures_cleanup(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "DROP PROCEDURE IF EXISTS pytest_proc_schema.test_proc1")
        cursor.execute(
            "DROP PROCEDURE IF EXISTS pytest_proc_schema.test_proc2")
        cursor.execute(
            "DROP PROCEDURE IF EXISTS pytest_proc_schema.test_params_proc")
        cursor.execute(
            "DROP PROCEDURE IF EXISTS pytest_proc_schema.test_no_results")
...
def test_foreignkeys_all(cursor, db_connection):
    ...
    try:
        ...
        fks = cursor.foreignKeys(
            table='orders', schema='pytest_fk_schema').fetchall()

        # Verify we got results
        assert fks is not None, "foreignKeys() should return results"
        assert len(
            fks) > 0, "foreignKeys() should return at least one foreign key"
...
def test_foreignkeys_specific_table(cursor, db_connection):
    ...
    try:
        ...
        # Verify we got results
        assert len(
            fks) == 1, "Should find exactly one foreign key for orders table"
...
def test_foreignkeys_specific_foreign_table(cursor, db_connection):
    ...
    try:
        ...
        fks = cursor.foreignKeys(
            foreignTable='customers', foreignSchema='pytest_fk_schema').fetchall()

        # Verify we got results
        assert len(
            fks) > 0, "Should find at least one foreign key referencing customers table"
...
def test_foreignkeys_both_tables(cursor, db_connection):
    ...
    try:
        ...
        # Verify we got results
        assert len(
            fks) == 1, "Should find exactly one foreign key between specified tables"
...
def test_foreignkeys_catalog_schema(cursor, db_connection):
    ...
    try:
        ...
        # Verify we got results
        assert len(
            fks) > 0, "Should find foreign keys with correct catalog/schema"
...
def test_foreignkeys_result_structure(cursor, db_connection):
    ...
    try:
        ...
        assert first_row.fkcolumn_name.lower(
        ) == 'customer_id', "Wrong foreign key column name"
        assert first_row.pkcolumn_name.lower(
        ) == 'customer_id', "Wrong primary key column name"
...
def test_foreignkeys_multiple_column_fk(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "DROP TABLE IF EXISTS pytest_fk_schema.product_variants")
        ...
        fks = cursor.foreignKeys(
            table='order_details', schema='pytest_fk_schema').fetchall()

        # Verify we got results
        assert len(
            fks) == 2, "Should find two rows for the composite foreign key (one per column)"
...
def test_primarykeys_setup(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "DROP TABLE IF EXISTS pytest_pk_schema.composite_pk_test")
...
def test_primarykeys_simple(cursor, db_connection):
    ...
    try:
        ...
        pks = cursor.primaryKeys(
            'single_pk_test', schema='pytest_pk_schema').fetchall()
...
def test_primarykeys_composite(cursor, db_connection):
    ...
    try:
        # Get primary key information
        pks = cursor.primaryKeys(
            'composite_pk_test', schema='pytest_pk_schema').fetchall()
        ...
        # Verify first column
        assert pks[0].table_name.lower(
        ) == 'composite_pk_test', "Wrong table name"
        assert pks[0].column_name.lower(
        ) == 'dept_id', "Wrong first primary key column name"
        assert pks[0].key_seq == 1, "Wrong key sequence number for first column"

        # Verify second column
        assert pks[1].table_name.lower(
        ) == 'composite_pk_test', "Wrong table name"
        assert pks[1].column_name.lower(
        ) == 'emp_id', "Wrong second primary key column name"
...
def test_primarykeys_column_info(cursor, db_connection):
    ...
    try:
        ...
        assert hasattr(
            pk, 'table_schem'), "Result should have table_schem column"
        assert hasattr(
            pk, 'table_name'), "Result should have table_name column"
        assert hasattr(
            pk, 'column_name'), "Result should have column_name column"
...
def test_primarykeys_catalog_filter(cursor, db_connection):
    ...
    try:
        ...
        pks = cursor.primaryKeys(
            'single_pk_test', catalog=current_db, schema='pytest_pk_schema').fetchall()
        ...
        fake_pks = cursor.primaryKeys(
            'single_pk_test', catalog='nonexistent_db_xyz123').fetchall()
        assert len(
            fake_pks) == 0, "Should return empty list for non-existent catalog"
...
def test_rowcount_guid_table(cursor, db_connection):
    ...
    try:
        ...
        assert cursor.rowcount == - \
            1, "Rowcount should be -1 after a SELECT statement (before fetch)"
...
def test_specialcolumns_setup(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "DROP TABLE IF EXISTS pytest_special_schema.timestamp_test")
        cursor.execute(
            "DROP TABLE IF EXISTS pytest_special_schema.multiple_unique_test")
        cursor.execute(
            "DROP TABLE IF EXISTS pytest_special_schema.identity_test")
...
def test_rowid_columns_basic(cursor, db_connection):
    ...
    try:
        ...
        assert len(
            rowid_cols) == 1, "Should find exactly one ROWID column (first column of PK)"

        # Verify column name in the results
        col = rowid_cols[0]
        assert col.column_name.lower(
        ) == 'id', "Primary key column should be included in ROWID results"

        # Verify result structure
        assert hasattr(col, 'scope'), "Result should have scope column"
        assert hasattr(
            col, 'column_name'), "Result should have column_name column"
        ...
        assert hasattr(
            col, 'column_size'), "Result should have column_size column"
        assert hasattr(
            col, 'buffer_length'), "Result should have buffer_length column"
        assert hasattr(
            col, 'decimal_digits'), "Result should have decimal_digits column"
        assert hasattr(
            col, 'pseudo_column'), "Result should have pseudo_column column"

        # The scope should be one of the valid values or NULL
        assert col.scope in [
            0, 1, 2, None], f"Invalid scope value: {col.scope}"

        # The pseudo_column should be one of the valid values
        assert col.pseudo_column in [
            0, 1, 2, None], f"Invalid pseudo_column value: {col.pseudo_column}"
...
def test_rowid_columns_identity(cursor, db_connection):
    ...
    try:
        ...
        assert len(
            rowid_cols) == 1, "Should find exactly one ROWID column (identity column as PK)"
...
def test_rowid_columns_composite(cursor, db_connection):
    ...
    try:
        ...
        assert len(
            rowid_cols) >= 1, "Should find at least one ROWID column (first column of PK)"
        ...
        if len(rowid_cols) > 1:
            ...
            for col in rowid_cols:
                assert col.column_name.lower() in [
                    'id', 'code'], "Only PK columns should be returned"
...
def test_rowid_columns_nonexistent(cursor):
    ...
    # Should return empty list, not error
    assert isinstance(
        rowid_cols, list), "Should return a list for non-existent table"
...
def test_rowid_columns_nullable(cursor, db_connection):
    ...
    try:
        ...
        assert len(
            rowid_cols_with_nullable) == 1, "Should return exactly one column (PK)"
        assert rowid_cols_with_nullable[0].column_name.lower(
        ) == 'id', "PK column should be returned"
        ...
        assert len(
            rowid_cols_no_nullable) == 0, "Should return empty list when nullable=False (ODBC API behavior)"

    except Exception as e:
        pytest.fail(f"rowIdColumns nullable test failed: {e}")
    finally:
        cursor.execute(
            "DROP TABLE IF EXISTS pytest_special_schema.nullable_test")
        db_connection.commit()
...
def test_rowver_columns_basic(cursor, db_connection):
    ...
    try:
        ...
        assert rowver_col.column_name.lower(
        ) == 'last_updated', "ROWVER column should be 'last_updated'"
        assert rowver_col.type_name.lower() in [
            'rowversion', 'timestamp'], "ROWVER column should have rowversion or timestamp type"

        # Verify result structure - allowing for NULL values
        assert hasattr(rowver_col, 'scope'), "Result should have scope column"
        assert hasattr(
            rowver_col, 'column_name'), "Result should have column_name column"
        assert hasattr(
            rowver_col, 'data_type'), "Result should have data_type column"
        assert hasattr(
            rowver_col, 'type_name'), "Result should have type_name column"
        assert hasattr(
            rowver_col, 'column_size'), "Result should have column_size column"
        assert hasattr(
            rowver_col, 'buffer_length'), "Result should have buffer_length column"
        assert hasattr(
            rowver_col, 'decimal_digits'), "Result should have decimal_digits column"
        assert hasattr(
            rowver_col, 'pseudo_column'), "Result should have pseudo_column column"

        # The scope should be one of the valid values or NULL
        assert rowver_col.scope in [
            0, 1, 2, None], f"Invalid scope value: {rowver_col.scope}"
...
def test_rowver_columns_nonexistent(cursor):
    ...
    # Should return empty list, not error
    assert isinstance(
        rowver_cols, list), "Should return a list for non-existent table"
...
def test_rowver_columns_nullable(cursor, db_connection):
    ...
    try:
        ...
        assert len(
            rowver_cols_with_nullable) == 1, "Should find exactly one ROWVER column"
        assert rowver_cols_with_nullable[0].column_name.lower(
        ) == 'ts', "ROWVERSION column should be included"
        ...
        assert len(
            rowver_cols_no_nullable) == 1, "Should find exactly one ROWVER column"
        assert rowver_cols_no_nullable[0].column_name.lower(
        ) == 'ts', "ROWVERSION column should be included even with nullable=False"
...
def test_specialcolumns_catalog_filter(cursor, db_connection):
    ...
    try:
        ...
        # Verify catalog filter worked
        assert len(
            rowid_cols) > 0, "Should find ROWID columns with correct catalog"
        ...
        assert len(
            fake_rowid_cols) == 0, "Should return empty list for non-existent catalog"
        ...
        # Verify catalog filter worked
        assert len(
            rowver_cols) > 0, "Should find ROWVER columns with correct catalog"
        ...
        assert len(
            fake_rowver_cols) == 0, "Should return empty list for non-existent catalog"
...
def test_statistics_setup(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "DROP TABLE IF EXISTS pytest_stats_schema.empty_stats_test")
...
def test_statistics_basic(cursor, db_connection):
    ...
    try:
        ...
        assert len(
            indexes) >= 3, "Should have at least 3 index entries (PK, unique email, non-unique dept+date)"
        ...
        assert hasattr(
            first_row, 'table_name'), "Result should have table_name column"
        assert hasattr(
            first_row, 'non_unique'), "Result should have non_unique column"
        assert hasattr(
            first_row, 'index_name'), "Result should have index_name column"
        assert hasattr(first_row, 'type'), "Result should have type column"
        assert hasattr(
            first_row, 'column_name'), "Result should have column_name column"
...
def test_statistics_unique_only(cursor, db_connection):
    ...
    try:
        ...
        assert len(
            stats) > 0, "statistics() with unique=True should return at least one row"

        # All index entries should be for unique indexes (non_unique = 0)
        for stat in stats:
            if hasattr(stat, 'type') and stat.type != 0:  # Skip TABLE_STAT entries
                assert hasattr(
                    stat, 'non_unique'), "Index entry should have non_unique column"
...
def test_statistics_empty_table(cursor, db_connection):
    ...
    try:
        ...
        assert len(
            stats) > 0, "statistics() should return at least one row for empty table"
...
def test_statistics_nonexistent(cursor):
    ...
    # Should return empty list, not error
    assert isinstance(
        stats, list), "Should return a list for non-existent table"
...
def test_statistics_result_structure(cursor, db_connection):
    ...
    try:
        ...
        for column in required_columns:
            assert hasattr(
                index_row, column), f"Result missing required column: {column}"

        # Check types of key columns
        assert isinstance(index_row.table_name,
                          str), "table_name should be a string"
...
def test_statistics_catalog_filter(cursor, db_connection):
    ...
    try:
        ...
        assert len(
            fake_stats) == 0, "Should return empty list for non-existent catalog"
...
def test_columns_setup(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "DROP TABLE IF EXISTS pytest_cols_schema.columns_special_test")
...
def test_columns_all(cursor, db_connection):
    ...
    try:
        ...
        assert hasattr(
            first_row, 'column_size'), "Result should have column_size column"
        assert hasattr(
            first_row, 'buffer_length'), "Result should have buffer_length column"
        assert hasattr(
            first_row, 'decimal_digits'), "Result should have decimal_digits column"
        assert hasattr(
            first_row, 'num_prec_radix'), "Result should have num_prec_radix column"
        assert hasattr(
            first_row, 'nullable'), "Result should have nullable column"
        assert hasattr(
            first_row, 'remarks'), "Result should have remarks column"
        assert hasattr(
            first_row, 'column_def'), "Result should have column_def column"
        assert hasattr(
            first_row, 'sql_data_type'), "Result should have sql_data_type column"
        assert hasattr(
            first_row, 'sql_datetime_sub'), "Result should have sql_datetime_sub column"
        assert hasattr(
            first_row, 'char_octet_length'), "Result should have char_octet_length column"
        assert hasattr(
            first_row, 'ordinal_position'), "Result should have ordinal_position column"
        assert hasattr(
            first_row, 'is_nullable'), "Result should have is_nullable column"
...
def test_columns_specific_table(cursor, db_connection):
    ...
    try:
        ...
        desc_col = next(
            col for col in cols if col.column_name.lower() == 'description')
...
def test_columns_special_chars(cursor, db_connection):
    ...
    try:
        ...
        # Verify we got results
        assert len(
            cols) == 9, "Should find exactly 9 columns in columns_special_test"
        ...
        col_names_lower = [
            name.lower() if name else None for name in col_names]
        ...
        assert any('user name' in name.lower()
                   for name in col_names), "Column with spaces should be in results"
        assert any('id' == name.lower()
                   for name in col_names), "ID column should be in results"
        assert any('123_numeric_start' in name.lower()
                   for name in col_names), "Column starting with numbers should be in results"
        assert any('max' == name.lower()
                   for name in col_names), "MAX column should be in results"
        assert any('select' == name.lower()
                   for name in col_names), "SELECT column should be in results"
        assert any('column.with.dots' in name.lower()
                   for name in col_names), "Column with dots should be in results"
        assert any('column/with/slashes' in name.lower()
                   for name in col_names), "Column with slashes should be in results"
        assert any('column_with_underscores' in name.lower()
                   for name in col_names), "Column with underscores should be in results"
...
def test_columns_specific_column(cursor, db_connection):
    ...
    try:
        ...
        assert col.table_schem.lower(
        ) == 'pytest_cols_schema', "Schema should be 'pytest_cols_schema'"
        ...
        assert pattern_cols[0].column_name.lower(
        ) == 'created_date', "Should find created_date column"
        ...
        assert len(
            multi_cols) >= 3, "Should find at least 3 columns matching '%d%'"
...
def test_columns_nonexistent(cursor):
    ...
    assert len(
        schema_cols) == 0, "Should return empty list for non-existent schema"
...
def test_columns_data_types(cursor):
    ...
    try:
        ...
        # INT column
        assert 'int' in col_dict['id'].type_name.lower(
...
def test_columns_catalog_filter(cursor):
    ...
    try:
        ...
        assert len(
            fake_cols) == 0, "Should return empty list for non-existent catalog"
...
def test_columns_schema_pattern(cursor):
    ...
    try:
        ...
        test_cols = [col for col in cols if col.table_name.lower()
                     == 'columns_test']
        ...
        assert len(
            test_cols) > 0, "Should find columns using specific schema pattern"
...
def test_columns_ordinal_position(cursor):
    ...
    try:
        ...
        assert sorted_cols[0].column_name.lower(
        ) == 'id', "First column should be id"
...
def test_executemany_with_uuids(cursor, db_connection):
    ...
    try:
        ...
        assert len(rows) == len(
            test_data), "Number of fetched rows does not match inserted rows."
        ...
        for retrieved_uuid, retrieved_desc in rows:
            ...
            if expected_uuid is None:
                ...
                assert isinstance(
                    retrieved_uuid, uuid.UUID), f"Expected UUID, got {type(retrieved_uuid)}"
...
def test_nvarcharmax_executemany_streaming(cursor, db_connection):
    ...
    try:
        ...
        # --- executemany insert ---
        cursor.executemany("INSERT INTO #pytest_nvarcharmax VALUES (?)", [
                           (v,) for v in values])
        db_connection.commit()
...
def test_varcharmax_executemany_streaming(cursor, db_connection):
    ...
    try:
        ...
        # --- executemany insert ---
        cursor.executemany("INSERT INTO #pytest_varcharmax VALUES (?)", [
                           (v,) for v in values])
        db_connection.commit()
...
def test_varbinarymax_executemany_streaming(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "CREATE TABLE #pytest_varbinarymax (col VARBINARY(MAX))")
        db_connection.commit()

        # --- executemany insert ---
        cursor.executemany("INSERT INTO #pytest_varbinarymax VALUES (?)", [
                           (v,) for v in values])
        db_connection.commit()

        # --- fetchall ---
        cursor.execute(
            "SELECT col FROM #pytest_varbinarymax ORDER BY DATALENGTH(col)")
        rows = [r[0] for r in cursor.fetchall()]
        assert rows == sorted(values, key=len)
...
def test_date_string_parameter_binding(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            f"SELECT a_column FROM {table_name} WHERE RIGHT(a_column, 10) = ?", (date_str,))
        rows = cursor.fetchall()

        assert rows == [
        ], f"Expected no match for date-like string, got {rows}"
...
def test_time_string_parameter_binding(cursor, db_connection):
    ...
    try:
        ...
        assert rows == [
        ], f"Expected no match for time-like string, got {rows}"
...
def test_datetime_string_parameter_binding(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            f"SELECT datetime_col FROM {table_name} WHERE datetime_col = ?", (datetime_str,))
        rows = cursor.fetchall()

        assert rows == [
        ], f"Expected no match for datetime-like string, got {rows}"
...
def test_numeric_basic_roundtrip(cursor, db_connection, precision, scale, value):
    ...
    try:
        cursor.execute(
            f"CREATE TABLE {table_name} (val NUMERIC({precision}, {scale}))")
        ...
        expected = value.quantize(decimal.Decimal(
            f"1e-{scale}")) if scale > 0 else value
...
def test_numeric_high_precision_roundtrip(cursor, db_connection, value):
    ...
    try:
        ...
        cursor.execute(f"SELECT val FROM {table_name}", scrollable=True)
        row = cursor.fetchone()
        assert row is not None
...
def test_numeric_null_and_multiple_rows(cursor, db_connection):
    ...
    try:
        ...
        values = [decimal.Decimal("123.45678"), None,
                  decimal.Decimal("-999.99999")]
        for v in values:
        ...
        assert any(
            r is None for r in rows), "Expected one NULL value in result set"
...
def test_numeric_precision_scale_positive_exponent(cursor, db_connection):
    try:
        ...
        db_connection.commit()
        cursor.execute("INSERT INTO #pytest_numeric_test (numeric_column) VALUES (?)", [
                       decimal.Decimal('31400')])
        db_connection.commit()
        cursor.execute("SELECT numeric_column FROM #pytest_numeric_test")
        row = cursor.fetchone()
        assert row[0] == decimal.Decimal(
            '31400'), "Numeric data parsing failed"
...
def test_numeric_precision_scale_negative_exponent(cursor, db_connection):
    try:
        ...
        row = cursor.fetchone()
        assert row[0] == decimal.Decimal(
            '0.03140'), "Numeric data parsing failed"
...
@pytest.mark.parametrize("values", [[
    decimal.Decimal("11.11"), decimal.Decimal(
        "22.22"), decimal.Decimal("33.33")
]])
def test_numeric_fetchmany(cursor, db_connection, values):
    ...
    try:
        ...
        assert all_rows == sorted(
            values), f"fetchmany mismatch: expected {sorted(values)}, got {all_rows}"
...
@pytest.mark.parametrize("values", [[
    decimal.Decimal("111.1111"), decimal.Decimal(
        "222.2222"), decimal.Decimal("333.3333"),
]])
def test_numeric_executemany(cursor, db_connection, values):
    ...
    try:
        ...
        cursor.executemany(
            f"INSERT INTO {table_name} (val) VALUES (?)", params)
        db_connection.commit()

        cursor.execute(f"SELECT val FROM {table_name} ORDER BY val")
        rows = [r[0] for r in cursor.fetchall()]
        assert rows == sorted(
            values), f"executemany() mismatch: expected {sorted(values)}, got {rows}"
...
@pytest.mark.parametrize("value, expected_precision, expected_scale", [
    ...
    (decimal.Decimal('000000123.45'), 38, 2),
    # Leading zeros in decimal part
    (decimal.Decimal('000.0001234'), 38, 7),
    # Many leading zeros + decimal
    (decimal.Decimal('0000000000000.123456789'), 38, 9),
    # Lots of leading zeros (avoiding E notation)
    (decimal.Decimal('000000.000000123456'), 38, 12)
])
...
@pytest.mark.parametrize("value", [
    ...
    # Close to maximum with integer part
    decimal.Decimal('1.' + '9' * 36),
])
def test_numeric_precision_boundary_limits(cursor, db_connection, value):
    ...
    try:
        ...
        pytest.skip(
            f"Value {value} may exceed SQL Server precision limits: {e}")
    finally:
        try:
            cursor.execute(f"DROP TABLE {table_name}")
            db_connection.commit()
...
@pytest.mark.parametrize("value, description", [
    # 39 digits
    (decimal.Decimal('1' + '0' * 38), "39 digits integer"),
    # 39 digits of 9s
    (decimal.Decimal('9' * 39), "39 nines"),
    ...
    (decimal.Decimal('0.111111111111111111111111111111111111111'),
     "39 decimal places"),  # 39 decimal digits
    (decimal.Decimal('1' * 20 + '.' + '9' * 20),
     "40 total digits"),                 # 40 total digits (20+20)
    (decimal.Decimal('123456789012345678901234567890.12345678901234567'),
     "47 total digits"),  # 47 total digits
])
...
LARGE_XML = "<root>" + \
    "".join(f"<item>{i}</item>" for i in range(10000)) + "</root>"
EMPTY_XML = ""
...
def test_xml_basic_insert_fetch(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #pytest_xml_basic (xml_col) VALUES (?);", SMALL_XML)
        db_connection.commit()

        row = cursor.execute(
            "SELECT xml_col FROM #pytest_xml_basic;").fetchone()
...
def test_xml_empty_and_null(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #pytest_xml_empty_null (xml_col) VALUES (?);", None)
        db_connection.commit()

        rows = [r[0] for r in cursor.execute(
            "SELECT xml_col FROM #pytest_xml_empty_null ORDER BY id;").fetchall()]
...
def test_xml_large_insert(cursor, db_connection):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO #pytest_xml_large (xml_col) VALUES (?);", LARGE_XML)
        db_connection.commit()

        row = cursor.execute(
            "SELECT xml_col FROM #pytest_xml_large;").fetchone()
...
def test_xml_batch_insert(cursor, db_connection):
    ...
    try:
        ...
        cursor.executemany("INSERT INTO #pytest_xml_batch (xml_col) VALUES (?);", [
                           (x,) for x in xmls])
        db_connection.commit()

        rows = [r[0] for r in cursor.execute(
            "SELECT xml_col FROM #pytest_xml_batch ORDER BY id;").fetchall()]
        assert rows == xmls
    finally:
