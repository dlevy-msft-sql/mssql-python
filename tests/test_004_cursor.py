...
def test_cursor_description_datetime_types(cursor, db_connection):
    ...
    try:
        ...
        # Test 2: Backwards compatible - compare with Python types (for pandas, etc.)
        assert desc[1][1] == datetime.date, "DATE should compare equal to datetime.date"
        assert desc[2][1] == datetime.time, "TIME should compare equal to datetime.time"
        assert desc[3][1] == datetime.datetime, "DATETIME should compare equal to datetime.datetime"
        assert (
            desc[4][1] == datetime.datetime
        ), "DATETIME2 should compare equal to datetime.datetime"
...
def test_column_metadata_thread_safety_concurrent_cursors(db_connection, conn_str):
    """
    Test thread safety of _column_metadata with multiple cursors in concurrent threads.

    Validates:
    - Multiple threads can safely create connections and cursors
    - Each cursor's _column_metadata remains isolated and valid
    - No race conditions between execute() setting metadata and fetchall() reading it

    This tests the _column_metadata instance attribute that is set during
    _initialize_description() and read during _build_converter_map().

    Note: Each thread uses its own connection because SQL Server doesn't support
    Multiple Active Result Sets (MARS) by default. The test still validates that
    _column_metadata works correctly under concurrent load.
    """
    import threading
    ...
    def worker(thread_id, table_suffix):
        """Worker that creates connection, cursor, executes queries, and verifies metadata."""
        # Each thread gets its own connection (required - SQL Server doesn't support MARS)
        thread_conn = None
        cursor = None
        try:
