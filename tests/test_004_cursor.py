...
def test_geography_invalid_wkt_parsing(cursor, db_connection):
    ...
    try:
        ...
        # Rollback any pending transaction from the error
        try:
            db_connection.rollback()
        except Exception:
            pass
...
def test_column_metadata_thread_safety_concurrent_cursors(db_connection, conn_str):
    """
    Test thread safety of _column_metadata with multiple cursors sharing one connection.

    Validates:
    - Multiple threads can safely create cursors on a shared connection
    ...
    # Create a shared connection for all threads
    shared_conn = connect(conn_str)
    ...
    def worker(thread_id, table_suffix):
        """Worker that creates cursor, executes queries, and verifies metadata."""
        try:
            # Create this thread's own cursor
